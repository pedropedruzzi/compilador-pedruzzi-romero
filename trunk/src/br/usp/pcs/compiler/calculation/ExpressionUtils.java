...
public class ExpressionUtils {
	...
	public static Expression applyUnaryOperators(Queue<TokenType> op1, Expression e) {
		if (!isPrimitiveType(e.getType()))
			throw new CompilationException("illegal operand type for unary operator: " + e.getType());
	...
	public static boolean isCompatibleInitializer(Type t1, Type t2) {
		if (t1 == PrimitiveType.intTypeInstance()) {
			...
			return (t2 instanceof Array) && 
				isCharType(((Array) t1).getInnerType()) && 
				isCharType(((Array) t2).getInnerType());
		} else return false;
	}
